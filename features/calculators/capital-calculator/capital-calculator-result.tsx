...
import type { CapitalResult } from './types';

type CapitalCalculatorResultProps = {
...
export function CapitalCalculatorResult({
  result,
  onReset,
}: CapitalCalculatorResultProps) {
  const t = useTranslations('pages.capitalCalculator.result');
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-baseline justify-center gap-2 py-4 pb-7">
        ...
        >
          {result
            ? formatCurrency(result.requiredPrincipal, locale)
            : PLACEHOLDER}
        </span>
      ...
      <div className="bg-brand-50 border-brand-100 rounded-xl border p-4">
        <div className="flex items-end justify-between gap-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            >
              {result
                ? `${formatPercent(result.effectiveApy)}%`
                : `${PLACEHOLDER}%`}
            </span>
          </div>
        </div>
